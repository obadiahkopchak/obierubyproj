...
class Player
	...
	def make_boards
		player_ind = 1
		while planyer_ind <= @total_players
			@where_i_shot.push(TargBoard.new(player_ind, @a, @b))
			player_ind += 1
		end
	end
	...
	def were_we_hit(target) #this is the new checker for whether one of my boats was hit
