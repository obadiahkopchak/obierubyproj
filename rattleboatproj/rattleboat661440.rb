...
def emergency_exit
	puts "would you like to exit (type y if you need to)"
...
class TargBoard
	...
	def initialize (board_owner, x_size, y_size)
		...
		@board_display = []
		@x_axis = ["01","02","03","04","05","06","07","08","09","10","11","12",
...
class Player
	...
	def initialize (player_number, board_dim_x, board_dim_y, total_players, max_targets)
...
#makeletterlist is the letter sanity check, providing bounds for where to shoot
...
#hide my wessels, hides individual points, not full vessels
#at this time I am having a hard time imagining a good boat method

	def place_piece
		puts "plase type a number between 1 and #{@board_size}"
		@boat_pos = gets.chomp.to_i
		if @boat_pos < 1 || @boat_pos > @board_size
			puts "sorry you chose a number not on the board, go again"
			place_piece
		end
	end
	...
	def hide_my_wessels 
		...
		while (jack < @max_targets) do
			place_piece
			@my_list_of_boats.push(@boat_pos)
			@my_list_of_boats.uniq!
			jack = @my_list_of_boats.length
			puts @my_list_of_boats
		end
#		puts "\e[H\e[2J"
	end
	...
	def make_target_boards
		@target_boards = []
		player_track = 1
		while player_track <= @total_players do
			if player_track != @player_number
				@target_boards.push(TargBoard.new(player_track,@dim_x,@dim_y))
			else
				#the rational here is a result of semi lazy coding
				# I didn't want to create an even more complicated
				#means of tracking players to shoot at
				#and I felt having an array where the target boards 
				#are in the correct location relative to the players
				#number, if there is a better solution please feel
				#free to tell me
	...
	def letter_adds_what(letter)
		...
		@move_choice = 0
			...
			@move_choice = 10*@dim_x
		elsif letter == "l"
			@move_choice = 11*@dim_x
		elsif letter == "m"
			@move_choice = 12*@dim_x
		elsif letter == "n"
			@move_choice = 13*@dim_x
		elsif letter == "o"
			@move_choice = 14*@dim_x
		elsif letter == "p"
			@move_choice = 15*@dim_x
		elsif letter == "q"
			@move_choice = 16*@dim_x
		elsif letter == "r"
			@move_choice = 17*@dim_x
		elsif letter == "s"
			@move_choice = 18*@dim_x
		elsif letter == "t"
			@move_choice = 19*@dim_x																	
		else
	...
	def choose_number
		puts "Please type a number between 1 and #{@dim_x}"
		@sel_num = gets.chomp.to_i
		if (@sel_num < 1) || (@sel_num > @dim_x)
...
class RattleBoatGame
	...
	def gets_x_axis
		...
		if (@a < @board_size_min) || (@a >@board_size_max)
			puts "sorry you went outside of the range #{@board_size_min}"
			print " and #{@board_size_max}"
			puts "try again"
			gets_x_axis
		end
	end
	...
	def max_shots
		@max_targets = @num_o_players + 3
		@max_shots = (@max_targets/2).to_i
		puts @max_shots
	end
	...
	def player_list
		@player_list = []
		tracker = 0
		while tracker < @num_o_players do
			a_name = @player_database[tracker].show_name
			@player_list.push(a_name)
			tracker += 1
		end
	end

	def set_max_dead
		@max_dead = @num_o_players - 1
	end
	...
	def players_turn(player_number)
		...
		@player_database[current_player].update_targeted_board(target_player_real_pos, 
		player_shot, the_symbol)
	end

	def kill_a_player(player_name)
	...
	def life_check
		...
		while counter < @num_o_players do
			teh_name = @player_database[counter].show_name
			boat_count = @player_database[counter].fleet_size
			if boat_count < 1
				kill_a_player(teh_name)
			end
			counter +=1
		end
	end

	def is_everyone_else_dead(player_number)

	end

	def how_many_shots(player_number)
		boat_count = @player_database[player_number].fleet_size
		if boat_count >= 0.7*@max_targets
			@shots_this_turn = @max_shots
		elsif (boat_count < 0.7*@max_targets) && (boat_count >= 0.5*@max_targets)
			@shots_this_turn = @max_shots - 2
		else
			@shots_this_turn = 1
		end
	end
	...
	def are_u_alive(player_number)
		...
		boat_count = @player_database[player_number].fleet_size
		shot_count = 0
		how_many_shots(player_number)
		if boat_count > 0
			puts "you have #{@shots_this_turn} chances to attack other players"
			while shot_count < @shots_this_turn
	...
	def play_game
		while @player_list.length > 1 do #|| (@num_o_dead != @max_dead) do
			counter = 0
		#	@num_o_dead = 0
			while counter < @num_o_players do
				are_u_alive(counter)
