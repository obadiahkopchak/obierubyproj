...
#max targets will eventually be a balancing act of number of players
#and the board size
	def max_targets
		@max_targets = @total_players + 2
	end
...
#hide my wessels, hides individual points, not full vessels
	...
	def hide_my_wessels 
		...
		while (jack < @max_targets) do
			...
			puts "the game will not let you 'stack pieces'"
			puts "try to make sure you choose a new number each time"
			puts "if you forget what you played it will be listed/"
			puts "the game won't let you add a repeat number"
 #you many need to add checks to ensure players don't waste spots
 #the method .uniq! will destroy all elements in an array 
 #that are not unique, in theory this can ensure you don't waste your moves
