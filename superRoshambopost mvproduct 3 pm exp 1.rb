#Super Roshambo post minimum viable product experiment, goal, making it so humans can't see the other's move
# there will be a player class, the player will have two main attributes, the ability to take an input
# currently this version of the game doesn't have an internal mechanism to prevent players from using lizard and spock
# in a more standard  rock paper scissors games, so uhh, that could be a problem, priority is low
# again this version is to, hopefully, allow the game to clear the screen after someone plays
#def clear_screen
#		if player_count >=1
#			puts "\e[H\e[2J"
#		else 
...
class Player
	...
	def initialize (player_number, game_select, player_quant)
		@player_number = player_number
		@game_select = game_select
		@player_moves = []
		@player_wins = 0
		@player_quant = player_quant
...
class RockPaperScissorsLizardSpock
	...
	def choose_game_type
		...
		puts "if you would like an explanation of the rules type rules, otherwise type something else and keep going"
	...
	def how_many_players
		...
		player_count.times do |i| 
			@player_list.push(Player.new(i+1, @game_type, player_count))
		end
		(2-player_count).times do |i|
			@player_list.push(Robot.new(i+1, @game_type, player_count))	
		end
	end
