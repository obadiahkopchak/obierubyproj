...
def make_move(chosen_move, symbol)
	if @move_list[chosen_move] != chosen_move
		@move_list[chosen_move] = chosen_move
		@game_board.insert((chosen_move-1), "#{symbol}")
		@game_board.delete_at(chosen_move)
		lookatboard
	elsif @move_list[chosen_move] == chosen_move
		puts "that move has been selected, try again"
		player_move_new(symbol)
	else
		puts "something went wrong"
	end
end

def player_move_new(symbol)
	...
	if (move_choice >= 1 && move_choice <=9)
		make_move(move_choice, symbol)

	elsif (move_choice <= 1 || move_choice >=9)
		puts "That move isn't allowed, please play again"
		player_move_new(symbol)
	else 
