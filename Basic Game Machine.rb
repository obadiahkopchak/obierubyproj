	...
	def two_player_game
		...
		def play_round
			...
			puts "player two will now go"
				unless @total_people == 2
					puts " "
				else
					puts "\e[H\e[2J"
				end #hopefully this will clear the screen after plays in two human games, other wise it should do 
			#nothing
...
class ChooseAGame
	attr_accessor :inputs, :banana,
	def initialize
		talk_to_player
		choose_game_type
	end
	def talk_to_player
		puts "Welcome to the Text Based Game Emporium"
		puts "I am going to ask some questions to figure out what kind of game we should be playing"
		puts "How many people would you like to play the game? (right now I can only handle up to 2 humans)"
		number_of_humans = gets.chomp.to_i
		##try to make this a while loop to keep asking until they give the correct number
		if number_of_humans == 2
			@human_count =2
		elsif number_of_humans == 1
			@human_count = 1
		elsif number_of_humans == 0
			@human_count = 0
		else run again
		end
